...
`cves.yaml` を廃止し JSON に移行したもの。YAMLコメントの代替として各フィールドの意味をここに記す。
...
| `nickname` | string | yes | 人間が読みやすい略称（例: `CopyFail`） |
...
バージョン比較は `packaging.version.Version` の意味論（PEP 440互換）で行う。
